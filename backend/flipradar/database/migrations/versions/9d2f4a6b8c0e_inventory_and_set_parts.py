...
def upgrade() -> None:
    op.create_table(
        ...
        sa.UniqueConstraint("lego_set_id", "element_id", name="set_element_unique"),
    )
    op.create_index(
        "ix_set_part_requirements_lego_set_id", "set_part_requirements", ["lego_set_id"]
    )
    op.create_index(
        "ix_set_part_requirements_element_id", "set_part_requirements", ["element_id"]
    )
    op.create_table(
        "inventory_items",
        sa.Column("id", sa.Uuid(), nullable=False),
        ...
        sa.ForeignKeyConstraint(["element_id"], ["elements.id"], ondelete="RESTRICT"),
        sa.PrimaryKeyConstraint("id"),
        sa.UniqueConstraint("user_id", "element_id", name="user_element_unique"),
    )
    ...
    op.create_table(
        ...
        sa.Column(
            "manual_adjustment", sa.Integer(), nullable=False, server_default="0"
        ),
        sa.Column("substitute_element_id", sa.Uuid(), nullable=True),
        sa.Column(
            "updated_at",
            sa.DateTime(timezone=True),
            server_default=sa.text("CURRENT_TIMESTAMP"),
        ...
        sa.ForeignKeyConstraint(
            ["substitute_element_id"], ["elements.id"], ondelete="RESTRICT"
        ),
        sa.PrimaryKeyConstraint("id"),
        sa.UniqueConstraint(
            "user_id", "requirement_id", name="user_requirement_unique"
        ),
    )
    op.create_index(
        "ix_checklist_adjustments_user_id", "checklist_adjustments", ["user_id"]
    )
    op.create_index(
        "ix_checklist_adjustments_requirement_id",
        "checklist_adjustments",
        ["requirement_id"],
    )
