...
def upgrade() -> None:
    op.add_column("mfa_challenges", sa.Column("security_question_id", sa.String(length=40), nullable=False, server_default="first_pet"))
    op.add_column("mfa_challenges", sa.Column("failed_attempt_count", sa.Integer(), nullable=False, server_default="0"))
    op.create_table(
    ...
    op.create_index("ix_mfa_security_questions_user_question", "mfa_security_questions", ["user_id", "question_id"], unique=True)


def downgrade() -> None:
    op.drop_index("ix_mfa_security_questions_user_question", table_name="mfa_security_questions")
