...
def upgrade() -> None:
    ...
    op.create_index(
        "ix_portfolios_user_created_at", "portfolios", ["user_id", "created_at"]
    )
    ...
    op.add_column(
        "portfolio_items", sa.Column("portfolio_id", sa.Uuid(), nullable=True)
    )
    ...
    with op.batch_alter_table("portfolio_items") as batch_op:
        ...
        batch_op.create_foreign_key(
            "fk_portfolio_items_portfolio_id_portfolios",
            "portfolios",
            ["portfolio_id"],
            ["id"],
            ondelete="RESTRICT",
...
def downgrade() -> None:
    with op.batch_alter_table("portfolio_items") as batch_op:
        ...
        batch_op.drop_constraint(
            "fk_portfolio_items_portfolio_id_portfolios", type_="foreignkey"
        )
