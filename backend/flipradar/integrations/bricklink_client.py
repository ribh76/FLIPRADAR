...
class BricklinkClient:
    ...
    def get_set_metadata(self, set_number: str) -> dict[str, Any]:
        ...
        return {
            "set_number": normalize_set_number(set_number), "name": item.get("name"),
            "theme": item.get("category_name") or f"BrickLink category {item.get('category_id', 'unknown')}",
            "subtheme": item.get("category_name"), "release_year": item.get("year_released"),
            "retirement_year": item.get("year_last_produced"), "piece_count": item.get("no_of_parts"),
            "minifig_count": item.get("no_of_minifigs"),
        }

    def get_set_price_snapshot(self, set_number: str) -> dict[str, Any]:
        guides = [self.get_price_guide("SET", self._set_id(set_number), condition) for condition in ("N", "U")]
        usable = [guide for guide in guides if guide]
        if not usable:
            raise BricklinkNotFoundError(f"BrickLink price guide not found: {set_number}")
        values = [Decimal(str(guide.get("avg_price"))) for guide in usable if guide.get("avg_price") is not None]
        if not values:
            raise BricklinkApiError("BrickLink returned no average price")
        primary = usable[0]
        lows = [Decimal(str(guide.get("min_price"))) for guide in usable if guide.get("min_price") is not None]
        highs = [Decimal(str(guide.get("max_price"))) for guide in usable if guide.get("max_price") is not None]
        return {"condition": "new", "currency": primary.get("currency_code", "USD"),
            "low_price": min(lows or values),
            "median_price": values[0], "average_price": sum(values) / len(values),
    ...
    def get_part_catalog_records(self, query: str) -> list[dict[str, Any]]:
        ...
        return [{"source": {"name": "BrickLink catalog", "url": self._catalog_url("P", part_number)},
            "category": {"id": category_id, "name": item.get("category_name") or f"BrickLink category {category_id}"},
            "color": {"id": "unknown", "name": "Unspecified"},
            "part": {"part_num": part_number, "name": name, "image_urls": [item["image_url"]] if item.get("image_url") else [], "first_known_year": item.get("year_released"), "market_price": price, "market_price_currency": (guide or {}).get("currency_code", "USD")},
            "element": {"element_id": part_number, "name": name, "first_known_year": item.get("year_released")}}]

    def get_price_guide(self, item_type: str, number: str, condition: str) -> dict[str, Any] | None:
        try:
            return self._get(f"/items/{item_type}/{quote(number, safe='')}/price", {"guide_type": "sold", "new_or_used": condition, "currency_code": "USD"})
        except BricklinkNotFoundError:
            return None

    def _get(self, path: str, params: dict[str, str] | None = None) -> dict[str, Any]:
        settings = get_settings().marketplace.bricklink
        if not settings.usable or not all((settings.consumer_key, settings.consumer_secret, settings.token_value, settings.token_secret)):
            raise BricklinkApiError("BrickLink API is not configured")
        url = f"{_BASE_URL}{path}"
        response = self._session.get(url, params=params, headers={"Authorization": self._authorization_header(url, params or {}, settings)}, timeout=settings.timeout_seconds)
        ...
        try:
            ...
            raise BricklinkApiError(f"BrickLink request failed: {response.text[:500]}") from exc
        if payload.get("meta", {}).get("code") not in (None, 200):
            raise BricklinkApiError(str(payload.get("meta", {}).get("message") or "BrickLink API rejected request"))
    ...
    def _authorization_header(url: str, params: dict[str, str], settings: Any) -> str:
        oauth = {"oauth_consumer_key": settings.consumer_key, "oauth_token": settings.token_value,
            "oauth_nonce": secrets.token_hex(16), "oauth_timestamp": str(int(time.time())),
            "oauth_signature_method": "HMAC-SHA1", "oauth_version": "1.0"}
        pieces = urlsplit(url)
        signature_params = list(parse_qsl(pieces.query, keep_blank_values=True)) + list(params.items()) + list(oauth.items())
        encoded = sorted((quote(str(k), safe="~"), quote(str(v), safe="~")) for k, v in signature_params)
        parameter_string = "&".join(f"{k}={v}" for k, v in encoded)
        base_url = f"{pieces.scheme}://{pieces.netloc}{pieces.path}"
        base = "&".join(("GET", quote(base_url, safe="~"), quote(parameter_string, safe="~")))
        ...
        return "OAuth " + ", ".join(f'{quote(k, safe="~")}="{quote(str(v), safe="~")}"' for k, v in sorted(oauth.items()))
        ...
        for condition in ("N", "U"):
            ...
            for index, detail in enumerate(guide.get("price_detail", [])):
                if not isinstance(detail, dict) or detail.get("unit_price") is None:
                    continue
                listings.append({"external_listing_id": f"price-guide:{set_id}:{condition}:{index}", "price": detail["unit_price"], "shipping_price": 0, "condition": condition, "title": f"LEGO {normalize_set_number(set_number)}", "listing_url": BricklinkClient._catalog_url("S", set_id), "currency": guide.get("currency_code", "USD"), "source_payload": detail})
