...
class EbayMarketplaceAdapter(MarketplaceAdapter):
    ...
    def fetch_listings(self, set_number: str) -> list[dict[str, Any]]:
        ...
        token = self._application_token(settings.api_key, settings.api_secret, settings.timeout_seconds)
        ...
        return self._tag_marketplace(
            [self._listing(item) for item in payload.get("itemSummaries", []) if isinstance(item, dict)]
        )
    ...
    def _listing(item: dict[str, Any]) -> dict[str, Any]:
        ...
        shipping = item.get("shippingOptions") if isinstance(item.get("shippingOptions"), list) else []
        shipping_cost = next((option.get("shippingCost", {}).get("value") for option in shipping if isinstance(option, dict) and isinstance(option.get("shippingCost"), dict)), 0)
        seller = item.get("seller") if isinstance(item.get("seller"), dict) else {}
        return {
            "external_listing_id": item.get("itemId"), "price": price.get("value"),
            "shipping_price": shipping_cost, "condition": item.get("condition"),
            "title": item.get("title"), "listing_url": item.get("itemWebUrl") or item.get("itemHref"),
            "seller": seller.get("username") or seller.get("feedbackScore"),
            "currency": price.get("currency"), "source_payload": item,
        }
    ...
    def _raise_for_status(response: requests.Response, operation: str) -> None:
        try:
            ...
            raise EbayApiError(f"eBay failed to {operation}: {response.text[:500]}") from exc
