...
def _mock_bricklink_detail(
    ...
    try:
        metadata = metadata_override or bricklink_client.fetch_set_metadata(set_number)
        latest_snapshot = bricklink_client.fetch_set_price_snapshot(set_number)
    except bricklink_client.MockBricklinkSetNotFoundError as exc:
        raise HTTPException(
