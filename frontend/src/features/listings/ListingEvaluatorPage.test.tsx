...
vi.mock("../../services/apiClient", () => ({
  apiClient: {
    listings: { evaluate: mocks.evaluate, analyze: mocks.analyze },
    portfolio: { addItem: mocks.addItem },
    watchlist: { addListing: mocks.addToWatchlist },
  },
...
describe("ListingEvaluatorPage", () => {
  it("retrieves an analysis and adds the listing to the purchase portfolio", async () => {
    ...
    mocks.evaluate.mockResolvedValue({
      id: "listing-1",
      title: "LEGO 75192 sealed complete",
      condition: "new",
      is_complete: true,
      is_verified: true,
      total_price: "520.00",
    ...
    mocks.analyze.mockResolvedValue({
      ...
      fair_value: "725.00",
      fair_value_low: "680.00",
      fair_value_high: "760.00",
      total_cost: "520.00",
      discount_percent: "28.30",
      premium_percent: "0.00",
      product_match_confidence: "100.00",
      decision: "buy",
      decision_confidence: "91.00",
      reasons: ["Strong value."],
      risk_flags: [],
      score_breakdown: { score: 88 },
      valuation_sample_size: 12,
      valuation_retrieved_at: "2026-08-06T12:00:00Z",
    ...
    await user.click(
      screen.getByRole("button", { name: /add to purchase portfolio/i }),
    );
    await waitFor(() =>
      expect(mocks.addItem).toHaveBeenCalledWith(
        expect.objectContaining({ set_number: "75192", purchase_price: 520 }),
      ),
    );
    await user.click(screen.getByRole("button", { name: "Save to watchlist" }));
    await waitFor(() =>
      expect(mocks.addToWatchlist).toHaveBeenCalledWith("listing-1"),
    );
  });
});
